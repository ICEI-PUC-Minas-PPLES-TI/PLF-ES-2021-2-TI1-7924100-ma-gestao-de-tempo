var indiceDbUsuarios, indiceDbTarefas = 0, indiceDbNotas = 0, tempoMin = calculaTempoMin(), qtdClicksTrocaMes = 0, addAno = 1, x = 1,
    usuarioAtual = JSON.parse(sessionStorage.getItem('usuarioCorrente')), idUsuarioAtual = usuarioAtual.id;
...
/*$.when( // função para carregar demais scripts
    ...
    window.onload = () => {
        ...
        if (window.screen.width <= 920 || document.body.clientWidth <= 920) {
            document.getElementById('asideBody').classList.add('hide');
        }
